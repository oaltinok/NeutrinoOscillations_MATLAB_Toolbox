function [check_par] = M_M_Range_Eps_et(distance,dirac_delta,NSI_delta,...
    h1,t1,...
    eps_et_min,eps_et_max,eps_et_inc,...
    eps_ee_nsi,...
    eps_tt_nsi,wb)
...
epsilon_et = eps_et_max;
epsilon_ee = eps_ee_nsi;
epsilon_tt = eps_tt_nsi;
eps_decrement=eps_et_inc;

waitbar(0.5,wb,'Making Calculations')

while(epsilon_et >= (eps_et_min-0.05))
...
if t1==1
    ...
    eps_et_legend{ii} = sprintf('\\epsilon_{e\\tau} = %2.1f', epsilon_et);
    ii = ii+1;
    epsilon_et = epsilon_et - eps_decrement;
...
%     ylim([0 1.0])
    ...
    % Create legend
for kk=1:length(eps_et_legend)
    set(plot1(kk),'DisplayName',eps_et_legend{kk});
end
